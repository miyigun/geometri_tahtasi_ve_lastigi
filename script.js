$(document).ready(function () {
    ...
    $('#resetBoardBtn').on('click', () => { clear3DElastics(); if (threeCamera) { const isMobile = window.innerWidth <= 640; threeCamera.position.set(0, 0, isMobile ? 13 : 9); threeControls && threeControls.reset(); } });
    ...
    function updateElastics3D() {
        ...
        elastics.forEach(el => {
            ...
            // Her kenar segmenti için ayrı düz tüp — keskin köşe garantisi
            for (let i = 0; i < points.length - 1; i++) {
                const segCurve = new THREE.LineCurve3(points[i], points[i + 1]);
                const segGeo = new THREE.TubeGeometry(segCurve, 2, 0.028, 8, false);
    ...
    function _commitGridPolygon(closed) {
        ...
        try {
            ...
            for (let i = 0; i < pts3D.length - 1; i++) {
                const segCurve = new THREE.LineCurve3(pts3D[i], pts3D[i + 1]);
                const segGeo = new THREE.TubeGeometry(segCurve, 2, 0.025, 8, false);
                const segMesh = new THREE.Mesh(segGeo, segMatBase.clone());
